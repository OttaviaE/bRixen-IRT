...
- si possono stime dei parametri distorte/non intepretabili
...
L’assunzione di \textbf{indipendenza locale} indica che non esiste alcuna relazione tra le risposte di un soggetto ad item diversi dopo aver controllato per il
tratto latente
...
Se si utilizza una procedura adattica (e.g., Computerized Adaptive Testing) $\rightarrow$ vincolare la somministrazione di uno solo dei due item
...
Dopo aver verificato le assuzioni, si può procedere alla verifica della fit del modello mediante due statistiche principali: 

- $M^2$ (Meyedeu-olivares & Joe, 2005): si basa sulla classificazione dei soggetti in base al loro pattern di risposta. Si basa sulla distribuzione $\chi^2$ per cui tende ad essere significativa per campioni ampi (anche se il modello ha una buona fit)

- Root Mean Square Error of Approximation (RMSEA): Misura di quanto il modello si avvicina alla realtà: 
  - $\leq .05$ Perfetto
  - $\leq .08$: Accettabile
  - $> .80$: No fit
...
1. \high{Massima verosimiglianza congiunta} (Joint Maximum Likelihood, JML): Permette di stimare per massima verosimiglianza \textbf{congiuntamente} i parametri degli item e delle persone

2. \high{Massima verosimiglianza marginale} (Marginal Maximum Likelihood, MML): Permette di stimare per massima verosimiglianza i parametri degli item, i parametri delle persone sono stimati successivamente con procedure bayesiane
...
- \high{\textbf{Expected a Posteriori}} (MAP): La stima del parametro è la media della distribuzione a posteriori di quel parametro
