...
$\theta_p$: abilità della persona (i.e., livello di tratto posseduto dalla persona) $\rightarrow$ maggiore $\theta_p$, maggiore il livello di tratto di $p$
      
$b_i$; difficoltà dell'item $i$ o location dell'item sul tratto latente $\rightarrow$ maggiore $b_i$, più è difficile rispondere correttamente a $i$ (\emph{endorse} $i$) 
...
SI può ottenere una misura della precisione con cui ogni item misura determinate parti del tratto latente $\rightarrow$ \emph{Item Information Function}: 
...
plot(theta, IRT(theta, a = -0.5, b = 0), 
     type = "l", cex.main = 3, 
     ylab = "P(x = 1)", ylim = c(0,1 ), cex.lab= 2, 
     cex.axis =1.5, xlab = expression(theta), lwd = 3, main = "Discriminatività negativa")
text(x= 3.2, 
     y = 0.95, expression(paste(a[1], "= -0.50")), 
     col = "black", cex = 2)
...
tratto è uguale alla difficoltà dell’item (in particolare, essa è $c + (1 - c)/2$)
...
Formalmente, il modello di Rasch è equivalente all'1PL, ma cambia la filosofia che c'è dietro.
...
Per intepretare la $-$\emph{2loglikelihood}, va calcolata la differenza tra la \emph{LogLikelihood} di due modelli nested moltiplicata per $-2$ ($-2$\emph{LogLikelihood}) e la differenza tra i gradi di libertà dei due modelli nested
...
Il BIC penalizza i modelli più complessi tenendo conto dell'ampiezza campionaria
